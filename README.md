...
```bash
./towstrap-server user add office --password 自己设的密码 --allow-ip 1.2.3.4
```
...
```
默认机器: office+default
agent token: tsa-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
在那台机器上执行 agent 安装命令：
  towstrap-agent --server wss://<服务器地址> --agent-token tsa-xxxx...
```
...
```bash
towstrap-agent --server wss://你的服务器:443 --agent-token tsa-xxxx...
```
...
```bash
./towstrap-server machine add office build   # 再拿一个 tsa-... token（会问 office 的密码做本人确认）
```
...
# 临时覆盖某一项（不改文件）：
./towstrap-server --config server.yaml --max-conns 100
```

SSH 主机密钥默认放在和 `users.db` 同目录的 `ssh_host_key`（即 `/etc/towstrap/ssh_host_key`），第一次启动自动生成；文件存在但读不了或解析失败会直接报错退出，**不会**静默换一把新钥匙——主机密钥一变，所有人的 ssh 客户端都会告警。
...
## 账号管理（都在服务器上的 towstrap-server 命令操作）

| 命令 | 作用 |
| --- | --- |
| `towstrap-server user add 名字 [--password 密码] [--contact 联系方式] [--allow-ip 地址]... [--agent-allow-ip 地址]... [--ssh-key 公钥]... [--ssh-key-file 文件]` | 建号，自动带一台 `default` 机器并打印它的 token 和安装命令 |
| `towstrap-server user list` | 列出账号、名下机器、白名单、TOTP 状态、公钥数、联系方式 |
| `towstrap-server user set 名字 [--password 密码] [--name 新名] [--contact 联系方式] [--allow-ip 地址]... [--agent-allow-ip 地址]... [--ssh-key 公钥]... [--ssh-key-file 文件] [--remove-ssh-key 公钥或SHA256指纹]... [--clear-ssh-keys] [--clear-allow] [--disable\|--enable]` | 改密码 / 改名 / 改备注 / 改白名单 / 增删公钥 / 停启用（`--agent-allow-ip` 只作用于唯一那台机器，多台时用 `machine set`） |
| `towstrap-server user remove 名字` | 删号，名下机器的 token 全部作废 |
| `towstrap-server user token 名字 [--regen] [--admin]` | 查看 token（仅当账号只有一台机器；多台时用 `machine token`）。要本人确认；`--regen` 是应急通道，必须加 `--admin` |
| `towstrap-server machine add 账号 机器名 [--agent-allow-ip 地址]... [--admin]` | 在账号下加一台机器，打印它的独立 token 和安装命令。要本人确认 |
| `towstrap-server machine list [账号]` | 列机器（账号、机器名、登录名、agent 白名单、token） |
| `towstrap-server machine set 账号 机器名 [--agent-allow-ip 地址]... [--clear-agent-allow]` | 改这一台机器的 agent 来源白名单 |
| `towstrap-server machine remove 账号 机器名` | 删一台机器，token 立刻作废 |
| `towstrap-server machine token 账号 机器名 [--regen] [--admin]` | 查看这一台机器的 token。要本人确认；`--regen` 是应急通道，必须加 `--admin` |
| `towstrap-server user totp 名字 [--remove]` | 绑定 / 解绑 TOTP 二因素 |
...
towstrap-agent token refresh                # 只换本机（会问账号密码；绑了 TOTP 再问验证码）
towstrap-agent token refresh --machine build        # 换同账号下的 build（它得在线）
...
账号存在 **SQLite 数据库**里（默认 `/etc/towstrap/users.db`，纯 Go 驱动，不需要装任何东西）：
...
```bash
towstrap-server user add office --allow-ip 1.2.3.4 --allow-ip 10.0.0.0/8   # 只有这些来源能 ssh office
towstrap-server user set office --clear-allow                               # 清空 = 不限来源
```
...
```bash
towstrap-server user add office --agent-allow-ip 203.0.113.0/24 ...   # 落到 default 机器上
towstrap-server user set office --agent-allow-ip 10.0.0.5             # 单机时照旧；多台报错
towstrap-server machine set office build --agent-allow-ip 10.0.0.5    # 多台时按台设置
towstrap-server machine set office build --clear-agent-allow          # 清空 = 不限来源
```
...
```bash
towstrap-server user totp office        # 打印 otpauth:// URI 和手动录入秘钥，输一次码确认绑定
towstrap-server user totp office --remove
```
...
```bash
towstrap-server user add office --contact ops@example.com ...
towstrap-server user set office --contact "值班：张三"
```
...
**不要把 towstrap 放在 Nginx / 云负载均衡这类反向代理后面。** 服务器是按 TCP 连接的来源 IP 做判断的，经过代理后所有连接的来源都变成代理自己的地址：IP 白名单、`--agent-allow-ip`、每 IP 连接数、防爆破限速、`AGENT-IPCHANGE` 告警全部失效。需要 HTTPS 直接用 `--tls`，需要挡在前面的话用四层透传（保留来源 IP 的 TCP 转发）或云防火墙。
...
```bash
towstrap-server user add bot --ssh-key "$(cat ~/.ssh/id_ed25519.pub)" --allow-ip 跑LLM的机器IP
# 或给已有账号补：towstrap-server user set bot --ssh-key-file ~/.ssh/id_ed25519.pub
```
...
| --- | --- | --- |
| MCP server 跑在 | 跑 LLM 应用的那台机器 B | towstrap 服务器 S 上 |
| B 上要装的东西 | `towstrap-mcp` 二进制 + SSH 私钥 | 不用装，填 URL + Bearer token |
| 执行通道 | B 经 SSH 登录 S，再落到 agent | S 内部直连 agent（Hub），不走 SSH 回环 |
| 能碰哪些机器 | mcp.yaml 里写死 | 每个客户端 token 各自的 `--machine` 列表 |
| 人工批准兜底 | `towstrap-mcp approve`（在 B 上跑） | `towstrap-server mcp approve`（在 S 上跑） |
| 适用 | 自己本机用 | 给别人/多台客户端用 |

### 方式一：stdio（towstrap-mcp）

**安装与配置**：`make build` 产出 `bin/towstrap-mcp`（发布包里是 `towstrap-mcp-<os>-<arch>`）。配置写在 `~/.config/towstrap/mcp.yaml`（`--config` 可换路径，`examples/mcp.yaml` 有全量注释模板）：服务器 SSH 入口、无口令私钥、known_hosts 或钉死的 `host_key` 指纹、机器列表（键就是 towstrap 账号名）、策略和限额。
...
```bash
towstrap-mcp pending          # 看谁在等
towstrap-mcp approve <id>     # 批准；--all 全批
towstrap-mcp deny <id>        # 拒绝；--all 全拒
```
...
```bash
towstrap-server mcp add laptop --machine bot            # bot 账号名下的全部机器
towstrap-server mcp add builder --machine alice+build   # 只给 alice+build 这一台
towstrap-server mcp add all-machines --machine '*'      # '*' = 全部机器（'alice+*' = alice 名下全部）
towstrap-server mcp list / set / remove / token         # 查看、改、删、换 token
towstrap-server mcp set laptop --disable                # 临时停用
```
...
内嵌模式下建议客户端支持批准弹窗（elicitation）；不支持时待批请求落在服务器的 `approvals_dir`，管理员在服务器上跑 `towstrap-server mcp pending / approve / deny` 兜底。机器范围收窄到 `machines=["other"]` 的客户端只能看到自己那几台。审计上：认证失败记 `MCP-AUTH-FAIL`，批准流转记 `MCP-ASK`/`MCP-APPROVED`/`MCP-DENIED`/`MCP-ASK-TIMEOUT`/`MCP-POLICY-DENY`，命令会话记 `SESSION-START mode=mcp from=mcp:客户端名@IP`。
...
仓库自带一份教编码助手「怎么安全地用 towstrap」的 skill（源文件在 `skills/towstrap/SKILL.md`）：什么时候用 MCP 工具、什么时候走 ssh、策略拒绝时不要去绕、哪些是禁区。装法有三条，任选：

**服务器直接提供**（不装 towstrap-mcp 也行，skill 原文挂在网页口的 `GET /skill`，无需口令）：
...
`towstrap-server mcp add` 创建客户端凭据时，输出末尾也会带上这几行命令。
...
towstrap-mcp connect uninstall          # 卸载（用户改过的内容默认不删，--force 才删）
```

支持：Claude Code、Codex、Grok Build、Cursor、Gemini CLI、OpenCode、GitHub Copilot CLI、Devin CLI（检测各自的家目录；Codex 和 Grok 共用 `~/.agents/skills`，只写一份）。安装是写入 `<skills目录>/towstrap/SKILL.md` 加一份 `.towstrap-managed.json` 清单；你自己放的同名 skill 不会被误盖（要 `--force`）。所有写操作都能先 `--dry-run` 演练。

**手工拷贝**：把 `skills/towstrap/` 整个目录拷进任何一家认识的 skills 目录，效果一样。

配套的还有 `connect print-mcp`：打印各家客户端接 towstrap MCP 要写的配置片段（不写文件）：
...
```bash
curl -H "X-Agent-Token: tsa-xxxx" https://服务器:443/status
...
towstrap 本质是远程控制：agent 装好后，持有账号密码的人随时能进这台机器。所以 agent **默认让机器的主人能感知到**，而不是悄悄运行：
...
- **审计日志**：agent 每次启动记一条 `AGENT-START`（时间、版本、连哪台服务器、shell、insecure/quiet 等参数）；每个远程会话的开始/结束各记一条（会话 id、来源 `登录账号@IP`）。默认位置 root 是 `/var/lib/towstrap/audit.log`，普通用户是 `~/.towstrap/audit.log`，`--audit-log` 可改。机器主人随时能查「这台机器被谁连过、agent 是怎么配的」。
- 通知发不出去（没桌面、权限没批）不影响会话；审计日志才是保底。

**别用 root 跑 agent**：远程会话拿到的是 agent 进程那个用户的 shell——agent 以 root 跑，每个连进来的人直接是 root。agent 启动时发现自己是 root 会打一条警告，`AGENT-START` 审计里也记 `uid=`。建议建一个专用低权限用户，`examples/towstrap-agent.service` 是现成的 systemd 单元（`User=towstrap`、`NoNewPrivileges` 等都配好了）。

想安静是显式动作，不是默认：`--quiet`（或 agent.yaml 的 `quiet: true`）只关通知，**审计日志照写**。

```bash
towstrap-agent ... --quiet                    # 关通知，审计仍写
towstrap-agent ... --audit-log /var/log/w2s.log  # 换审计路径
```

## 注意

- **配置怎么给**：长久配置写 `server.yaml` / `agent.yaml`（`examples/` 里有全量注释模板，`--config` 加载，命令行旗标显式给过的项覆盖文件）；秘密（agent token）优先用 `--agent-token-file`（0600 文件）或环境变量 `TOWSTRAP_AGENT_TOKEN`，别把 token 写进配置文件。
...
- **token 别走命令行**：`--agent-token tsa-...` 会出现在 `ps` 里。用 `--agent-token-file 路径`（文件权限 0600）或环境变量 `TOWSTRAP_AGENT_TOKEN`，配置文件 `agent_token` 也行。agent 给远程会话起 shell 时会把 `TOWSTRAP_AGENT_TOKEN` 从环境里去掉，SSH 进来的人 `env` 看不到它。
...
- `towstrap-server-<os>-<arch>` — 服务器端 + 账号管理（`user` 子命令）
- `towstrap-agent-<os>-<arch>` — 被控机上的 agent
- `towstrap-mcp-<os>-<arch>` — MCP 入口，装在跑 LLM 应用的机器上（见「MCP」节）
...
minisign -Vm towstrap-agent-darwin-arm64          # 有签名文件时
```
