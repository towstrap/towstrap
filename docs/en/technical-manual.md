...
**Persistent sessions**: with `session`, `run_command` skips one-shot exec and uses a `shellSession` (`internal/mcpsrv/shell.go`) holding a long-lived non-PTY shell — the server opens an exec session with `open` + empty `cmd` + the `nx` flag and keeps stdin (stdio mode uses an SSH `Shell()` channel preceded by a `TOWSTRAP_MCP_SHELL=1` env request so the server forwards `nx`). Every line written to the shell is `__tsec=0; eval 'literal command' || __tsec=$?; printf SENTINEL`: the command sits inside single quotes handed to `eval`, so embedded newlines stay inside the quotes and a truncated quote/syntax fragment can at most make eval fail — it can never linger in shell input waiting for the next command to complete it (this is the boundary that keeps a denied command from running later); `||` captures the failure code and immunizes the line against `set -e`. A session-random prefix + increasing sequence stamps the marker on both stdout and stderr; pump goroutines require a complete "marker + exit code + newline" — prefixes echoed by `set -x` or colliding with output don't count. Commands serialize inside a session; timeout/close kills the **whole process group** (the shell's running foreground command and `&` background jobs go together — use `setsid` to deliberately detach a daemon; on Windows only the main process is killed); `exit`/`exec`/disconnect marks the session dead and the next same-named call starts a fresh shell with `session_restarted` in the result. With `nx` + zsh the agent launches `zsh +o nomatch +o banghist` (startup flags, effective before the first command is read — an stdin init line would be too late): nomatch otherwise treats an unmatched glob (the `items[0]`-style text LLMs constantly emit) as fatal to the whole non-interactive shell, and banghist would expand `!` from history. A policy-denied or cwd-rejected command writes zero bytes into the shell. Keyed `machine\x00session`, scoped to the MCP client session — closed when it ends; `session_idle` reaps idle shells, `max_sessions` caps the count. Runners not implementing `ShellOpener` return a clear error for `session`. Audited as `MCP-SESSION-CMD` plus agent-side `mode=mcp-shell`.
